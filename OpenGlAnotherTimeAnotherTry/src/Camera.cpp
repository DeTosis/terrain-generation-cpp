...
glm::mat4 Camera::CameraLookMatrix(const float& xPos, const float& yPos)
{
	glm::mat4 view;
...
void Camera::CameraMovement(GLFWwindow* window, float& deltaTime, float& speed)
{
...
void Camera::UpdateCursorLockState(GLFWwindow* window)
{
	...
	if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS && !isHovered)
	{
